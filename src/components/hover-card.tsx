...
const HoverCard = ({ title, subtitle, Icon }: CardType) => {
  return (
    <div className="w-full p-4 rounded border-[1px] border-slate-300 relative overflow-hidden group bg-white flex flex-col items-center justify-center">
      ...
      <h3 className="font-medium text-xl text-slate-950 group-hover:text-white relative duration-300">
        {title}
      </h3>
      <p className="text-slate-400 group-hover:text-violet-200 relative duration-300">{subtitle}</p>
    </div>
  )
}
